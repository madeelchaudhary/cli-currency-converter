...
async function startGame() {
  do {
    ...
    if (userChosenOperation === "Help") {
      ...
      const userChosenCurrency = await askCurrency(
        currencies,
      ...
      const userChosenBaseCurrency = await askCurrency(
        currencies,
        "Select the base currency."
      );
      const userChosenTargetCurrency = await askCurrency(
        currencies,
      ...
      const exchangedAmount = await pairConvert(
        userChosenBaseCurrency,
        userChosenTargetCurrency,
        amountToExchange
      );
      if (exchangedAmount) {
        spinner.success({ text: "Succeed" });
        logNote(
          `${amountToExchange} ${userChosenBaseCurrency} = ${exchangedAmount} ${userChosenTargetCurrency}`
        );
      } else {
        spinner.error({ text: "something went wrong!" });
      }
